...
\[
S_d = \sun_{k=1}^{N-1} L_d(q_k, \dot{q_{k+1}})
  \]
  Where
\[
...
\left(
  \frac{\partial L}{\partial (\partial_\mu \varphi_a)}\!\left(\varphi, \varphi_t, \varphi_x, \varphi_y\right)
\right)
= 0
\quad\text{for all } a.
...
\begin{alighn}
  \int_{x_i}^{x_{i+1}} dx \int_{y_i}^{y_{i+1}}dy L(\varphi, \varphi_t, \varpi_x, \varphi_y)
&\approx
L_h\!\Big(
    \varphi_{i,j}(t), \varphi_{i+1,j}(t), \varphi_{i+1,j+1}(t), \varphi_{i,j+1}(t), \\
&\qquad\quad
