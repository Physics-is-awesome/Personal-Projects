...
module mod_dg
  ...
  real(dp), allocatable :: fh1(:,:,:,:), fh2(:,:,:,:), fh3(:,:,:,:)
...
contains
  ...
  subroutine dg_init()
    ...
    allocate(fh1(nqf,2,3,ncell), fh2(nvar,nqf,2,3), fh3(nqf,2,3,ncell))
  ...
  subroutine dg_destroy()
    ...
    if (allocated(fh1))    deallocate(fh1, fh2, fh3)
  ...
  subroutine dg_residual(Y, t, res)
    ...
    if (trim(scheme) /= 'sp') then
       ...
       block
         real(dp) :: rc(nb,ncell)
         rc = Y(:,IRHO,:)
         if (ieq == EQ_CONST) rc = rc - rhoe        ! Jeans: source is rho-rho0
         call solve_rho(rc, bcm, phi_h, g_h, its, resid)
       end block
    ...
    block
      real(dp) :: rdel(nb,ncell)
      rdel = Y(:,IRHO,:) - rhoe
      phid = zero
      call solve_rho(rdel, BCP_ZERO, phid, gd, its, resid)
    end block
    poisson_iters = its ; poisson_resid = resid

    phi_h = phie + phid
    g_h   = ge   + gd
    call dg_U_from_W(Y, Y(:,IRHO,:), phi_h, U)   ! (3.32)
    ...
    !--- Step 2 : modified HLLC flux (3.33), stored face by face -------
    fh1 = zero ; fh3 = zero
    do ic = 1, ncell
       do m = 1, nd
          do s = 1, 2
             ...
             do q = 1, nqf
                do iv = 1, nvar
                   ul(iv) = eval_f(U(:,iv,ic), q, s, m)
                end do
                call outer_state(ic, jc, q, s, m, U, ul, ur)
    ...
    do ic = 1, ncell
       ...
       rbar  = U(1,IRHO,ic)             ! (rho_h)_K  cell average
       rebar = rhoe(1,ic)               ! (rho^e_h)_K

       !------------- volume: F(W).grad v  and  F_g.grad v -------------
       do q = 1, nqv
          do iv = 1, nvar
             uq(iv) = eval_v(U(:,iv,ic), q)
          end do
          do m = 1, nd
             call phys_flux(uq, m, fq)
             do iv = 1, nvar
       ...
       do m = 1, nd
          ...
          do s = 1, 2
             ...
             do q = 1, nqf
                ...
                ! (a) HLLC flux (recomputed traces stored in fh1/fh3 and
                !     rebuilt here for the momentum components)
                do iv = 1, nvar
                   ul(iv) = eval_f(U(:,iv,ic), q, s, m)
                end do
                call outer_state(ic, jc, q, s, m, U, ul, ur)
  ...
  contains
    ...
    subroutine store_face_flux(icc)
