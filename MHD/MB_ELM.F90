...
! 2D flux-coordinate grid (psi, theta), implicit time-stepping, GMRES solver
...
program elm_mhd_advanced
  ...
  integer, parameter :: npsi = 200, ntheta = 100      ! Grid points
  real(8), parameter :: dpsi = (psi_max-psi_min)/npsi, dtheta = (theta_max-theta_min)/ntheta
  ...
  do t = 1, n_steps
     ...
     ! Implicit solve (placeholder: GMRES)
     ...
     ! Output diagnostics
     if (mod(t, 100) == 0 .and. rank == 0) then
...
subroutine initialize_fields(rho, m_psi, m_theta, s, B_psi, B_theta, eta, mu_visc, kappa, D, &
  ...
  do i = 1, npsi
     do j = 1, ntheta
        psi = psi_min + (i-1) * (psi_max-psi_min)/(npsi-1)
        theta = theta_min + (j-1) * (theta_max-theta_min)/(ntheta-1)
        r = abs(psi - psi_sep)  ! Distance from separatrix
        ! Pedestal-like profile with ELM perturbation
        rho(i,j) = 1.0 + 0.5 * exp(-r**2 / pedestal_width**2) + 0.1 * sin(5*theta) * exp(-r**2)
        s(i,j) = 1.0 + 0.3 * exp(-r**2 / pedestal_width**2) + 0.05 * sin(5*theta) * exp(-r**2)
        m_psi(i,j) = 0.0
        m_theta(i,j) = 0.0
        B_psi(i,j) = B0 * cos(0.1 * r)
        B_theta(i,j) = B0 * sin(0.1 * r)
        ! Spatially varying transport coefficients (higher in SOL)
        eta(i,j) = eta_0 * (1.0 + 10.0 * exp(-r**2 / (2*pedestal_width**2)))
        mu_visc(i,j) = mu_0_visc * (1.0 + 10.0 * exp(-r**2 / (2*pedestal_width**2)))
        kappa(i,j) = kappa_0 * (1.0 + 10.0 * exp(-r**2 / (2*pedestal_width**2)))
        D(i,j) = D_0 * (1.0 + 10.0 * exp(-r**2 / (2*pedestal_width**2)))
     end do
  end do
...
subroutine compute_derived(rho, m_psi, m_theta, s, u_psi, u_theta, T, p, mu_chem, npsi, ntheta)
  ...
  do i = 1, npsi
     do j = 1, ntheta
        u_psi(i,j) = m_psi(i,j) / rho(i,j)
        u_theta(i,j) = m_theta(i,j) / rho(i,j)
        T(i,j) = s(i,j) / rho(i,j)  ! Simplified EOS
        p(i,j) = rho(i,j) * T(i,j)
...
subroutine compute_poisson(rho, m_psi, m_theta, s, B_psi, B_theta, u_psi, u_theta, p, &
  ...
  real(8) :: div_rho_u(npsi,ntheta), div_m_u(npsi,ntheta), div_BB(npsi,ntheta), curl_uB(npsi,ntheta)
  integer :: i, j
  ...
  do i = 2, npsi-1
     do j = 2, ntheta-1
        ! div(rho*u)
        div_rho_u(i,j) = (rho(i+1,j)*u_psi(i+1,j) - rho(i-1,j)*u_psi(i-1,j))/(2*dpsi) + &
                         (rho(i,j+1)*u_theta(i,j+1) - rho(i,j-1)*u_theta(i,j-1))/(2*dtheta)
        ! div(m*u + (p + |B|^2/2mu_0)*I - B*B/mu_0)
        div_m_u(i,j) = (m_psi(i+1,j)*u_psi(i+1,j) - m_psi(i-1,j)*u_psi(i-1,j))/(2*dpsi) + &
                       (m_psi(i,j+1)*u_theta(i,j+1) - m_psi(i,j-1)*u_theta(i,j-1))/(2*dtheta) + &
                       (p(i+1,j) + 0.5*(B_psi(i+1,j)**2 + B_theta(i+1,j)**2)/mu_0 - &
                        (p(i-1,j) + 0.5*(B_psi(i-1,j)**2 + B_theta(i-1,j)**2)/mu_0))/(2*dpsi) - &
                       (B_psi(i+1,j)*B_psi(i+1,j) - B_psi(i-1,j)*B_psi(i-1,j))/(2*dpsi*mu_0)
        ! div(s*u)
        ds_dt(i,j) = (s(i+1,j)*u_psi(i+1,j) - s(i-1,j)*u_psi(i-1,j))/(2*dpsi) + &
                     (s(i,j+1)*u_theta(i,j+1) - s(i,j-1)*u_theta(i,j-1))/(2*dtheta)
        ! curl(u x B)
        curl_uB(i,j) = ( (u_theta(i,j+1)*B_psi(i,j+1) - u_theta(i,j-1)*B_psi(i,j-1))/(2*dtheta) - &
                         (u_psi(i+1,j)*B_theta(i+1,j) - u_psi(i-1,j)*B_theta(i-1,j))/(2*dpsi) )
     end do
  end do
...
subroutine compute_4bracket(rho, m_psi, m_theta, s, B_psi, B_theta, u_psi, u_theta, T, mu_chem, &
  ...
  do i = 2, npsi-1
     do j = 2, ntheta-1
        grad_ux(1,i,j) = (u_psi(i+1,j) - u_psi(i-1,j))/(2*dpsi)
        grad_ux(2,i,j) = (u_psi(i,j+1) - u_psi(i,j-1))/(2*dtheta)
        grad_uy(1,i,j) = (u_theta(i+1,j) - u_theta(i-1,j))/(2*dpsi)
        grad_uy(2,i,j) = (u_theta(i,j+1) - u_theta(i,j-1))/(2*dtheta)
        grad_T(1,i,j) = (T(i+1,j) - T(i-1,j))/(2*dpsi)
        grad_T(2,i,j) = (T(i,j+1) - T(i,j-1))/(2*dtheta)
        grad_mu(1,i,j) = (mu_chem(i+1,j) - mu_chem(i-1,j))/(2*dpsi)
        grad_mu(2,i,j) = (mu_chem(i,j+1) - mu_chem(i,j-1))/(2*dtheta)
        curl_B(i,j) = (B_theta(i+1,j) - B_theta(i-1,j))/(2*dpsi) - (B_psi(i,j+1) - B_psi(i,j-1))/(2*dtheta)
     end do
  end do

  do i = 2, npsi-1
     do j = 2, ntheta-1
        tau(1,1,i,j) = mu_visc(i,j) * (2 * grad_ux(1,i,j) - (2.0/3.0) * (grad_ux(1,i,j) + grad_uy(2,i,j)))
        tau(1,2,i,j) = mu_visc(i,j) * (grad_ux(2,i,j) + grad_uy(1,i,j))
        tau(2,1,i,j) = tau(1,2,i,j)
        tau(2,2,i,j) = mu_visc(i,j) * (2 * grad_uy(2,i,j) - (2.0/3.0) * (grad_ux(1,i,j) + grad_uy(2,i,j)))
  ...
  do i = 2, npsi-1
     do j = 2, ntheta-1
        div_J_rho(i,j) = (J_rho(1,i+1,j) - J_rho(1,i-1,j))/(2*dpsi) + (J_rho(2,i,j+1) - J_rho(2,i,j-1))/(2*dtheta)
        div_tau(i,j) = (tau(1,1,i+1,j) - tau(1,1,i-1,j))/(2*dpsi) + (tau(1,2,i,j+1) - tau(1,2,i,j-1))/(2*dtheta)
        div_q(i,j) = (q(1,i+1,j) - q(1,i-1,j))/(2*dpsi) + (q(2,i,j+1) - q(2,i,j-1))/(2*dtheta)
        curl_J(i,j) = (J(2,i+1,j) - J(2,i-1,j))/(2*dpsi) - (J(1,i,j+1) - J(1,i,j-1))/(2*dtheta)
     end do
  end do
  ...
  ds_dt = ds_dt - div_q/(rho*T) + (mu_visc(i,j)/T(i,j) * sum(grad_ux(:,i,j)*grad_ux(:,i,j) + &
                                   grad_uy(:,i,j)*grad_uy(:,i,j)) + &
                                   eta(i,j)/(mu_0**2 * T(i,j)) * curl_B(i,j)**2 + &
                                   D(i,j)/T(i,j) * sum(grad_mu(:,i,j)*grad_mu(:,i,j)))/T(i,j)
...
subroutine compute_energy_entropy(rho, m_psi, m_theta, s, B_psi, B_theta, T, grad_ux, grad_T, &
  ...
  do i = 2, npsi-1
     do j = 2, ntheta-1
        e = T(i,j)
        energy = energy + (0.5 * (m_psi(i,j)**2 + m_theta(i,j)**2)/rho(i,j) + &
                          rho(i,j) * e + 0.5 * (B_psi(i,j)**2 + B_theta(i,j)**2)/mu_0) * dpsi * dtheta
        entropy = entropy + s(i,j) * dpsi * dtheta
        entropy_prod = entropy_prod + (D(i,j)/T(i,j) * sum(grad_mu(:,i,j)**2) + &
                                      mu_visc(i,j)/T(i,j) * sum(grad_ux(:,i,j)**2) + &
                                      kappa(i,j)/(T(i,j)**2) * sum(grad_T(:,i,j)**2) + &
                                      eta(i,j)/(mu_0**2 * T(i,j)) * curl_B(i,j)**2) * rho(i,j) * T(i,j) * dpsi * dtheta
     end do
  end do
...
subroutine save_fields(rho, m_psi, m_theta, s, B_psi, B_theta, npsi, ntheta)
  ...
  do i = 1, npsi
     do j = 1, ntheta
        write(10, *) i, j, rho(i,j), m_psi(i,j), m_theta(i,j), s(i,j), B_psi(i,j), B_theta(i,j)
     end do
  end do
  close(10)
