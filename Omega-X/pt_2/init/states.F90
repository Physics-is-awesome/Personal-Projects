...
#include "../config/dim_config.h"
  ...
  real, allocate :: mass(nx), temp(nx), entropy(nx), momentum(nx)
#elif dim == 2
  real, allocatable :: mass(:,:), temp(:,:), entropy(:,:), momentum(:,:)
  real, allocate :: mass(nx, ny), temp(nx, ny), entropy(nx, ny), momentum(nx, ny)
#elif dim == 3
  real, allocatable :: mass(:,:,:), temp(:,:,:), entropy(:,:,:), momentum(:,:,:)
  real, allocate :: mass(nx, ny, nz), temp(nx, ny, nz), entropy(nx, ny, nz), momentum(nx, ny, nz)
#endif
  call read_file()
  call init_temp()
  print*, temp(1, 2, 3)
